...
public:
    ...
    std::ostream& displaySorted(std::ostream& out, std::function<bool(const T&)> filter,
    ...
    template<typename T1, typename T2> friend std::ostream& operator<<(std::ostream&, const Heap<T1, T2>&);
};   // class Heap


template<typename T1, typename T2> std::ostream& operator<<(std::ostream& out, const Heap<T1, T2>& h)
{
    out << "[ ";
    if (h.count) {
        out << h.data[0];
