...
template<typename T, typename I, typename C> class IntruHeap
{

    static_assert(std::is_same<IntruHeapData&, typename std::result_of<I(T&)>::type>::value,
...
public:
    ...
    void adjust_up(T& item) { sift_up(intru_data_of(item)); }

    void adjust_down(T& item) { sift_down(intru_data_of(item)); }

    void adjust(T& item) { sift(intru_data_of(item)); }
    ...
    std::ostream& display_sorted(std::ostream& out, bool insert_line_breaks = true,
