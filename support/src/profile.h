...
namespace crimson {
template<typename T> class ProfileBase
{
...
public:
    ...
    {
        if (0 == count) return nan("");
        double variance = (count * sum_squares - sum * sum) / double(count * count);
...
template<typename T> class ProfileCombiner;


template<typename T> class ProfileTimer : public ProfileBase<T>
{
...
public:
    ...
    void stop()
    {
        ...
        if (0 == this->count) {
            ...
            if (duration_count < this->low)
                this->low = duration_count;
            else if (duration_count > this->high)
                this->high = duration_count;
        }
        ++this->count;
        is_timing = false;
...
template<typename T> class ProfileCombiner : public ProfileBase<T>
{
...
public:
    ...
    void combine(const ProfileTimer<T>& timer)
    {
        if (0 == this->count) {
            ...
            if (timer.low < this->low)
                this->low = timer.low;
            else if (timer.high > this->high)
                this->high = timer.high;
        }
