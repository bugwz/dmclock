 ...
 */
template<typename I, typename T, IndIntruHeapData T::*heap_info, typename C, unsigned K = 2> class IndIntruHeap
{

    // shorthand
    using HeapIndex = IndIntruHeapData;

    static_assert(std::is_same<T, typename std::pointer_traits<I>::element_type>::value,
                  "class I must resolve to class T by indirection (pointer dereference)");

    static_assert(std::is_same<bool, typename std::result_of<C(const T&, const T&)>::type>::value,
    ...
    public:
        ...
        bool operator==(const Iterator& other) const { return heap == other.heap && index == other.index; }
    ...
    class ConstIterator
    {
        ...
        ConstIterator(const IndIntruHeap<I, T, heap_info, C, K>& _heap, HeapIndex _index)
            : heap(&_heap)
            , index(_index)
        {
            // empty
    ...
    public:
        ...
        bool operator==(const ConstIterator& other) const { return heap == other.heap && index == other.index; }

        bool operator!=(const ConstIterator& other) const { return !(*this == other); }

        const T& operator*() { return *heap->data[index]; }
...
public:
    ...
    Iterator at(const I& ind_item)
    {
        ...
        if (ind >= count) {
            throw std::out_of_range(std::to_string(ind) + " >= " + std::to_string(count));
        }
        assert(data[ind] == ind_item);
        return Iterator(*this, ind);
    }

    void promote(T& item) { sift_up(item.*heap_info); }

    void demote(T& item) { sift_down(item.*heap_info); }

    void adjust(T& item) { sift(item.*heap_info); }
    ...
    std::ostream& display_sorted(std::ostream& out, std::function<bool(const T&)> filter = all_filter) const
    {
        static_assert(std::is_copy_constructible<I>::value,
                      "cannot call display_sorted when class I is not copy"
                      " constructible");
        auto compare = [this](const I first, const I second) -> bool { return this->comparator(*first, *second); };
...
protected:
    static IndIntruHeapData& intru_data_of(const I& item) { return (*item).*heap_info; }
    ...
    // parameter
    template<bool EnableBool = true> typename std::enable_if<(K > 2) && EnableBool, void>::type sift_down(HeapIndex i)
    {
        if (i >= count) return;
        while (true) {
            HeapIndex li = lhs(i);
