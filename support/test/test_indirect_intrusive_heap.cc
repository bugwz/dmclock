...
struct Elem
{
    ...
    bool operator==(const Elem& other) const { return data == other.data; }
...
struct ElemCompare
{
    bool operator()(const Elem& d1, const Elem& d2) const { return d1.data < d2.data; }
};
...
{
    crimson::IndIntruHeap<std::shared_ptr<Elem>, Elem, &Elem::heap_data, ElemCompare, 3> heap;
...
{
    crimson::IndIntruHeap<std::shared_ptr<Elem>, Elem, &Elem::heap_data, ElemCompare, 4> heap;
...
{
    crimson::IndIntruHeap<std::shared_ptr<Elem>, Elem, &Elem::heap_data, ElemCompare, 10> heap;
...
{
    crimson::IndIntruHeap<std::shared_ptr<Elem>, Elem, &Elem::heap_data, ElemCompare, 2> heap2;

    crimson::IndIntruHeap<std::shared_ptr<Elem>, Elem, &Elem::heap_data, ElemCompare, 3> heap3;

    crimson::IndIntruHeap<std::shared_ptr<Elem>, Elem, &Elem::heap_data, ElemCompare, 4> heap4;

    crimson::IndIntruHeap<std::shared_ptr<Elem>, Elem, &Elem::heap_data, ElemCompare, 10> heap10;
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        EXPECT_GE(current, bound) << "we should never go down, only increase or remain the same";
        EXPECT_EQ(current, heap3.top().data) << "heap1's data and heap3's data should match";
        EXPECT_EQ(current, heap4.top().data) << "heap1's data and heap4's data should match";
        EXPECT_EQ(current, heap10.top().data) << "heap1's data and heap10's data should match";
...
TEST(IndIntruHeap, remove_careful)
{
    ...
    crimson::IndIntruHeap<std::shared_ptr<Elem>, Elem, &Elem::heap_data, ElemCompare, 2> heap;
    ...
    EXPECT_NE(heap.end(), k) << "we should have found an element with the value 200, which we'll remove";
    heap.remove(k);
    ...
    EXPECT_EQ(10, i->data);
    ++i;
    EXPECT_EQ(40, i->data) << "this needs to be 40 or there's a mistake in implementation";
    ++i;
    EXPECT_EQ(20, i->data);
    ++i;
    EXPECT_EQ(30, i->data);
    ++i;
    EXPECT_EQ(100, i->data) << "this needs to be 100 or there's a mistake in implementation";
}
...
TEST(IndIntruHeap, remove_greatest)
{
    ...
    for (auto i : toremove) {
        ...
        EXPECT_NE(heap.end(), k) << "we should have found an element with the value 300, which we'll remove";
        heap.remove(k);
    }
...
TEST_F(HeapFixture1, iterator_basics)
{
...

    {
        ...
        for (auto i = heap.begin(); i != heap.end(); ++i) {
            auto v = *i;
            EXPECT_NE(values.end(), values.find(v.data)) << "value in heap must be part of original set";
            values.erase(v.data);
        }
...
{
    {
        ...
        EXPECT_NE(heap.end(), it1) << "find by indirection for included element should succeed";
        EXPECT_EQ(-7, it1->data) << "find by indirection for included element should result in right value";

        auto fake_data = std::make_shared<Elem>(-7);
        auto it2 = heap.find(fake_data);
        EXPECT_EQ(heap.end(), it2) << "find by indirection for not included element should fail";
    }

    {
        auto it1 = heap.find(Elem(-7));
        EXPECT_NE(heap.end(), it1) << "find by value for included element should succeed";
        EXPECT_EQ(-7, it1->data) << "find by value for included element should result in right value";

        auto it2 = heap.find(Elem(7));
        EXPECT_EQ(heap.end(), it2) << "find by value for not included element should fail";
    }

    {
        auto it1 = heap.rfind(data7);
        EXPECT_NE(heap.end(), it1) << "reverse find by indirecton for included element should succeed";
        EXPECT_EQ(-7, it1->data) << "reverse find by indirection for included element should result "
                                    "in right value";

        auto fake_data = std::make_shared<Elem>(-7);
        auto it2 = heap.rfind(fake_data);
        EXPECT_EQ(heap.end(), it2) << "reverse find by indirection for not included element should fail";
    }

    {
        auto it1 = heap.rfind(Elem(-7));
        EXPECT_NE(heap.end(), it1) << "reverse find by value for included element should succeed";
        EXPECT_EQ(-7, it1->data) << "reverse find by value for included element should result "
                                    "in right value";

        auto it2 = heap.rfind(Elem(7));
        EXPECT_EQ(heap.end(), it2) << "reverse find by value for not included element should fail";
    }
}
...
TEST_F(HeapFixture1, const_iterator_find_rfind)
{
...

    {
        ...
        EXPECT_NE(c_heap.cend(), it1) << "find by indirection for included element should succeed";
        EXPECT_EQ(-7, it1->data) << "find by indirection for included element should result in right value";

        auto fake_data = std::make_shared<Elem>(-7);
        auto it2 = c_heap.find(fake_data);
        EXPECT_EQ(c_heap.cend(), it2) << "find by indirection for not included element should fail";
    }

    {
        auto it1 = c_heap.find(Elem(-7));
        EXPECT_NE(c_heap.cend(), it1) << "find by value for included element should succeed";
        EXPECT_EQ(-7, it1->data) << "find by value for included element should result in right value";

        auto it2 = c_heap.find(Elem(7));
        EXPECT_EQ(c_heap.cend(), it2) << "find by value for not included element should fail";
    }

    {
        auto it1 = c_heap.rfind(data7);
        EXPECT_NE(c_heap.cend(), it1) << "reverse find by indirecton for included element should succeed";
        EXPECT_EQ(-7, it1->data) << "reverse find by indirection for included element should result "
                                    "in right value";

        auto fake_data = std::make_shared<Elem>(-7);
        auto it2 = c_heap.rfind(fake_data);
        EXPECT_EQ(c_heap.cend(), it2) << "reverse find by indirection for not included element should fail";
    }

    {
        auto it1 = c_heap.rfind(Elem(-7));
        EXPECT_NE(c_heap.cend(), it1) << "reverse find by value for included element should succeed";
        EXPECT_EQ(-7, it1->data) << "reverse find by value for included element should result "
                                    "in right value";

        auto it2 = c_heap.rfind(Elem(7));
        EXPECT_EQ(c_heap.cend(), it2) << "reverse find by value for not included element should fail";
    }
}
...
TEST_F(HeapFixture1, iterator_remove)
{
    ...
    for (auto it3 = heap.begin(); it3 != heap.end(); ++it3) {
        EXPECT_NE(-7, it3->data) << "iterating through heap should not find removed value";
    }
