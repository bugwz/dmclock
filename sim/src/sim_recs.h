...
template<typename T> void time_stats(std::mutex& mtx, T& time_accumulate, std::function<void()> code)
{
...
// explicit
template<typename T, typename R> R time_stats_w_return(std::mutex& mtx, T& time_accumulate, std::function<R()> code)
{
    auto t1 = std::chrono::steady_clock::now();
    R result = code();
    auto t2 = std::chrono::steady_clock::now();
...
template<typename T> void count_stats(std::mutex& mtx, T& counter)
{
    std::lock_guard<std::mutex> lock(mtx);
    ++counter;
}
