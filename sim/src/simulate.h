...
template<typename ServerId, typename ClientId, typename TS, typename TC> class Simulation
{
...
public:
    ...
    using ServerDataOutF =
        std::function<void(std::ostream& out, Simulation* sim, ServerFilter, int header_w, int data_w, int data_prec)>;

    using ClientDataOutF =
        std::function<void(std::ostream& out, Simulation* sim, ClientFilter, int header_w, int data_w, int data_prec)>;
    ...
    const ServerId& get_server_id(std::size_t index) const { return server_ids[index]; }


    void add_servers(unsigned count, std::function<TS*(ServerId)> create_server_f)
    {
    ...
    void add_clients(unsigned count, std::function<TC*(ClientId)> create_client_f)
    {
    ...
    void run()
    {
        ...
        std::cout << "simulation completed in "
                  << std::chrono::duration_cast<std::chrono::milliseconds>(clients_finished_time - servers_created_time)
                         .count()
                  << " millisecs" << std::endl;
    ...
    void display_stats(
        std::ostream& out, ServerDataOutF server_out_f, ClientDataOutF client_out_f,
        ServerFilter server_filter = [](const ServerId&) { return true; },
        ClientFilter client_filter = [](const ClientId&) { return true; }, int head_w = 12, int data_w = 7,
        int data_prec = 2)
    {
        assert(has_run);
        ...
        double ops_factor = std::chrono::duration_cast<std::chrono::duration<double>>(measure_unit) /
        ...
        for (auto const& c : clients) {
            ...
            while (it != end && *it < start_edge) {
                ++it;
            }

            for (auto time_edge = start_edge + measure_unit; time_edge <= latest_finish + measure_unit;
...

        {
            ...
            do {
                ...
                for (auto const& c : clients) {
                    ...
                    out << " " << std::setw(data_w) << std::setprecision(data_prec) << std::fixed << data;
                }
                out << " " << std::setw(data_w) << std::setprecision(data_prec) << std::fixed << total << std::endl;
                ++i;
            } while (has_data);
        }
        ...
        display_client_internal_stats<std::chrono::nanoseconds>(out, "nanoseconds");
        ...
        display_server_internal_stats<std::chrono::nanoseconds>(out, "nanoseconds");
    ...
    template<typename T> void display_server_internal_stats(std::ostream& out, const std::string& time_unit)
    {
        ...
        for (unsigned i = 0; i < get_server_count(); ++i) {
            ...
            add_request_time += std::chrono::duration_cast<T>(is.add_request_time);
            request_complete_time += std::chrono::duration_cast<T>(is.request_complete_time);
            add_request_count += is.add_request_count;
            request_complete_count += is.request_complete_count;
        }

        double add_request_time_per_unit = double(add_request_time.count()) / add_request_count;
        out << "total time to add requests: " << std::fixed << add_request_time.count() << " " << time_unit << ";"
            << std::endl
            << "    count: " << add_request_count << ";" << std::endl
            << "    average: " << add_request_time_per_unit << " " << time_unit << " per request/response" << std::endl;

        double request_complete_time_unit = double(request_complete_time.count()) / request_complete_count;
        out << "total time to note requests complete: " << std::fixed << request_complete_time.count() << " "
            << time_unit << ";" << std::endl
            << "    count: " << request_complete_count << ";" << std::endl
            << "    average: " << request_complete_time_unit << " " << time_unit << " per request/response"
            << std::endl;

        out << std::endl;

        assert(add_request_count == request_complete_count);
        out << "server timing for QOS algorithm: " << add_request_time_per_unit + request_complete_time_unit << " "
            << time_unit << " per request/response" << std::endl;
    }


    template<typename T> void display_client_internal_stats(std::ostream& out, const std::string& time_unit)
    {
        T track_resp_time(0);
        ...
        for (unsigned i = 0; i < get_client_count(); ++i) {
            ...
            track_resp_time += std::chrono::duration_cast<T>(is.track_resp_time);
            get_req_params_time += std::chrono::duration_cast<T>(is.get_req_params_time);
            track_resp_count += is.track_resp_count;
            get_req_params_count += is.get_req_params_count;
        }

        double track_resp_time_unit = double(track_resp_time.count()) / track_resp_count;
        out << "total time to track responses: " << std::fixed << track_resp_time.count() << " " << time_unit << ";"
            << std::endl
            << "    count: " << track_resp_count << ";" << std::endl
            << "    average: " << track_resp_time_unit << " " << time_unit << " per request/response" << std::endl;

        double get_req_params_time_unit = double(get_req_params_time.count()) / get_req_params_count;
        out << "total time to get request parameters: " << std::fixed << get_req_params_time.count() << " " << time_unit
            << ";" << std::endl
            << "    count: " << get_req_params_count << ";" << std::endl
            << "    average: " << get_req_params_time_unit << " " << time_unit << " per request/response" << std::endl;

        out << std::endl;

        assert(track_resp_count == get_req_params_count);
        out << "client timing for QOS algorithm: " << track_resp_time_unit + get_req_params_time_unit << " "
            << time_unit << " per request/response" << std::endl;
    }
    ...
    const ServerId& server_select_0(uint64_t seed, uint16_t client_idx) { return server_ids[0]; }
};   // class Simulation
