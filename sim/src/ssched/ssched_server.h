...
public:
    ...
    using HandleRequestFunc = std::function<void(const C&, RequestRef, NullData, uint64_t)>;
...
public:
    ...
    SimpleQueue(CanHandleRequestFunc _can_handle_f, HandleRequestFunc _handle_f)
    ...
    ~SimpleQueue() { finishing = true; }

    void add_request(R&& request, const C& client_id, const ReqParams& req_params, uint64_t request_cost)
    {
        add_request(RequestRef(new R(std::move(request))), client_id, req_params, request_cost);
    }

    void add_request(RequestRef&& request, const C& client_id, const ReqParams& req_params, uint64_t request_cost)
    {
    ...
    PullReq pull_request()
    {
        ...
        if (queue.empty()) {
            ...
            result.data = typename PullReq::Retn{front.client, std::move(front.request)};
            queue.pop();
        }
