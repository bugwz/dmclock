...
struct CliInst
{
    ...
    CliInst(wait_op_t, D duration)
        : op(CliOp::wait)
    {
        args.wait_time = std::chrono::duration_cast<std::chrono::milliseconds>(duration);
    }

    CliInst(req_op_t, uint32_t count, double ops_per_sec, uint16_t max_outstanding)
        : op(CliOp::req)
    {
...
template<typename SvcTrk, typename ReqPm, typename RespPm, typename Accum> class SimulatedClient
{
public:
    ...
    using SubmitFunc = std::function<void(const ServerId&, TestRequest&&, const ClientId&, const ReqPm&)>;
...
public:
    SimulatedClient(ClientId _id, const SubmitFunc& _submit_f, const ServerSelectFunc& _server_select_f,
                    const ClientAccumFunc& _accum_f, const std::vector<CliInst>& _instrs)
        : id(_id)
        , submit_f(_submit_f)
    ...
    SimulatedClient(ClientId _id, const SubmitFunc& _submit_f, const ServerSelectFunc& _server_select_f,
                    const ClientAccumFunc& _accum_f, uint16_t _ops_to_run, double _iops_goal,
                    uint16_t _outstanding_ops_allowed)
        : SimulatedClient(_id, _submit_f, _server_select_f, _accum_f,
    ...
    virtual ~SimulatedClient() { wait_until_done(); }

    void receive_response(const TestResponse& resp, const ServerId& server_id, const RespPm& resp_params,
                          const Cost request_cost)
    {
        RespGuard g(mtx_resp);
        resp_queue.push_back(RespQueueItem{resp, server_id, resp_params, request_cost});
        cv_resp.notify_one();
    }
...
protected:
        ...
        for (auto i : instructions) {
            if (CliOp::wait == i.op) {
                ...
                for (uint64_t o = 0; o < i.args.req_params.count; ++o) {
                    ...
                    ReqPm rp = time_stats_w_return<decltype(internal_stats.get_req_params_time), ReqPm>(
                        internal_stats.mtx, internal_stats.get_req_params_time, [&]() -> ReqPm {
                            return service_tracker.get_req_params(server);
                        });
                    count_stats(internal_stats.mtx, internal_stats.get_req_params_count);

                    submit_f(server, TestRequest{server, static_cast<uint32_t>(o), 12}, id, rp);
                    ++outstanding_ops;
    ...
    void run_resp()
    {
	    ...
	    TestResponse& resp = item.response;
                ...
                time_stats(internal_stats.mtx, internal_stats.track_resp_time, [&]() {
                    service_tracker.track_resp(item.server_id, item.resp_params, item.request_cost);
                });
                count_stats(internal_stats.mtx, internal_stats.track_resp_count);
