...
namespace crimson {
namespace test_dmc {
void server_data(std::ostream& out, test::MySim* sim, test::MySim::ServerFilter server_disp_filter, int head_w,
                 int data_w, int data_prec);

void client_data(std::ostream& out, test::MySim* sim, test::MySim::ClientFilter client_disp_filter, int head_w,
                 int data_w, int data_prec);
}   // namespace test_dmc
}   // namespace crimson
...
int main(int argc, char* argv[])
{
    ...
    for (unsigned i = 0; i < client_groups; ++i) {
        client_info.push_back(test::dmc::ClientInfo{
            cli_group[i].client_reservation, cli_group[i].client_weight, cli_group[i].client_limit});
    }
    ...
    auto client_disp_filter = [=](const ClientId& i) -> bool { return i < 3 || i >= (client_total_count - 3); };

    auto server_disp_filter = [=](const ServerId& i) -> bool { return i < 3 || i >= (server_total_count - 3); };
    ...
    test::SubmitFunc server_post_f = [&simulation, &cli_group, &ret_client_group_f](
    ...
    test::DmcServer::ClientRespFunc client_response_f = [&simulation](ClientId client_id,
                                                                      const sim::TestResponse& resp,
                                                                      const ServerId& server_id,
                                                                      const dmc::PhaseType& phase,
                                                                      const sim::Cost request_cost) {
        simulation->get_client(client_id).receive_response(resp, server_id, phase, request_cost);
    };

    test::CreateQueueF create_queue_f = [&](test::DmcQueue::CanHandleRequestFunc can_f,
    ...
    auto create_client_f = [&](ClientId id) -> test::DmcClient* {
        ...
        return new test::DmcClient(
            id, server_post_f, std::bind(server_select_f, _1, id), test::dmc_client_accumulate_f, cli_inst[i]);
    };
    ...
    simulation->display_stats(
        std::cout, &test::server_data, &test::client_data, server_disp_filter, client_disp_filter);

    delete simulation;
}   // main


void test::client_data(std::ostream& out, test::MySim* sim, test::MySim::ClientFilter client_disp_filter, int head_w,
                       int data_w, int data_prec)
{
    ...
    out << " " << std::setw(data_w) << std::setprecision(data_prec) << std::fixed << total_r << std::endl;
    ...
    out << " " << std::setw(data_w) << std::setprecision(data_prec) << std::fixed << total_p << std::endl;
}


void test::server_data(std::ostream& out, test::MySim* sim, test::MySim::ServerFilter server_disp_filter, int head_w,
                       int data_w, int data_prec)
{
    out << std::setw(head_w) << "res_ops:";
    int total_r = 0;
    ...
    out << std::endl << " k-way heap: " << q.get_heap_branching_factor() << std::endl << std::endl;
    ...
    out << "Server add_request_timer: count:" << art_combiner.get_count() << ", mean:" << art_combiner.get_mean()
        << ", std_dev:" << art_combiner.get_std_dev() << ", low:" << art_combiner.get_low()
        << ", high:" << art_combiner.get_high() << std::endl;
    out << "Server request_complete_timer: count:" << rct_combiner.get_count() << ", mean:" << rct_combiner.get_mean()
        << ", std_dev:" << rct_combiner.get_std_dev() << ", low:" << rct_combiner.get_low()
        << ", high:" << rct_combiner.get_high() << std::endl;
    out << "Server combined mean: " << (art_combiner.get_mean() + rct_combiner.get_mean()) << std::endl;
#endif
}
