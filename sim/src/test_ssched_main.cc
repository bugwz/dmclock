...
int main(int argc, char* argv[])
{
    ...
    auto client_disp_filter = [=](const ClientId& i) -> bool { return i < 3 || i >= (client_count - 3); };

    auto server_disp_filter = [=](const ServerId& i) -> bool { return i < 3 || i >= (server_count - 3); };


    test::MySim* simulation;

    // lambda to post a request to the identified server; called by client
    test::SubmitFunc server_post_f = [&simulation](const ServerId& server_id,
    ...
    static std::vector<sim::CliInst> wait = {{sim::wait_op, client_wait},
                                             {sim::req_op, client_total_ops, client_iops_goal, client_outstanding_ops}};
    ...
    test::SimpleServer::ClientRespFunc client_response_f = [&simulation](ClientId client_id,
                                                                         const sim::TestResponse& resp,
                                                                         const ServerId& server_id,
                                                                         const ssched::NullData& resp_params,
                                                                         Cost request_cost) {
        simulation->get_client(client_id).receive_response(resp, server_id, resp_params, request_cost);
    };

    test::CreateQueueF create_queue_f = [&](test::SimpleQueue::CanHandleRequestFunc can_f,
    ...
    auto create_server_f = [&](ServerId id) -> test::SimpleServer* {
        return new test::SimpleServer(
            id, server_iops, server_threads, client_response_f, test::simple_server_accumulate_f, create_queue_f);
    };
                       ...
                       int data_w, int data_prec)
{
    ...
    out << std::setw(data_w) << std::setprecision(data_prec) << std::fixed << total_req << std::endl;
