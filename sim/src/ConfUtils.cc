...
////////////////////////////// ConfLine //////////////////////////////
ConfLine::ConfLine(const std::string& key_, const std::string& val_, const std::string& newsection_,
                   const std::string& comment_, int line_no_)
    : key(key_)
    , val(val_)
...
bool ConfLine::operator<(const ConfLine& rhs) const
{
...
{
    oss << "ConfLine(key = '" << l.key << "', val='" << l.val << "', newsection='" << l.newsection << "')";
    return oss;
}
///////////////////////// ConfFile //////////////////////////
ConfFile::ConfFile() {}
 ...
 */
int ConfFile::parse_file(const std::string& fname, std::deque<std::string>* errors, std::ostream* warnings)
{
    clear();
    ...
    if (st_buf.st_size > MAX_CONFIG_FILE_SZ) {
        ostringstream oss;
        oss << "read_conf: config file '" << fname << "' is " << st_buf.st_size << " bytes, but the maximum is "
            << MAX_CONFIG_FILE_SZ;
        errors->push_back(oss.str());
        ret = -EINVAL;
        goto done;
    ...
    if (fread(buf, 1, sz, fp) != sz) {
        if (ferror(fp)) {
            ret = -errno;
            ostringstream oss;
            oss << "read_conf: fread error while reading '" << fname << "': " << strerror_r(ret, buf2, sizeof(buf2));
            errors->push_back(oss.str());
            goto done;
        }
        else {
...
int ConfFile::read(const std::string& section, const std::string& key, std::string& val) const
{
    ...
    if (s == sections.end()) return -ENOENT;
    ConfLine exemplar(k, "", "", "", 0);
    ConfSection::const_line_iter_t l = s->second.lines.find(exemplar);
    if (l == s->second.lines.end()) return -ENOENT;
    val = l->val;
    return 0;
}

ConfFile::const_section_iter_t ConfFile::sections_begin() const
{
    return sections.begin();
}

ConfFile::const_section_iter_t ConfFile::sections_end() const
{
    return sections.end();
}
...
{
    for (ConfFile::const_section_iter_t s = cf.sections_begin(); s != cf.sections_end(); ++s) {
        oss << "[" << s->first << "]\n";
        for (ConfSection::const_line_iter_t l = s->second.lines.begin(); l != s->second.lines.end(); ++l) {
            if (!l->key.empty()) {
                oss << "\t" << l->key << " = \"" << l->val << "\"\n";
            }
        }
...
void ConfFile::load_from_buffer(const char* buf, size_t sz, std::deque<std::string>* errors, std::ostream* warnings)
{
    errors->clear();
 ...
 */
ConfLine* ConfFile::process_line(int line_no, const char* line, std::deque<std::string>* errors)
{
    ...
    while (true) {
        ...
        case ACCEPT_INIT:
            if (c == '\0')
                ...
                ostringstream oss;
                oss << "unexpected right bracket at char " << (l - line) << ", line " << line_no;
                errors->push_back(oss.str());
                return NULL;
            }
            else if (isspace(c)) {
        ...
        case ACCEPT_KEY:
            if ((((c == '#') || (c == ';')) && (!escaping)) || (c == '\0')) {
                ostringstream oss;
                if (c == '\0') {
                    oss << "end of key=val line " << line_no << " reached, no \"=val\" found...missing =?";
                }
                else {
        ...
        case ACCEPT_COMMENT_START:
            if (c == '\0') {
                ...
                ostringstream oss;
                oss << "unexpected character at char " << (l - line) << " of line " << line_no;
                errors->push_back(oss.str());
                return NULL;
            }
            break;
