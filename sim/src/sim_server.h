...
template<typename Q, typename ReqPm, typename RespPm, typename Accum> class SimulatedServer
{
    ...
    struct QueueItem
    {
        ...
        QueueItem(const ClientId& _client, std::unique_ptr<TestRequest>&& _request, const RespPm& _additional,
...
public:
    ...
    using ClientRespFunc =
        std::function<void(ClientId, const TestResponse&, const ServerId&, const RespPm&, const Cost)>;

    using ServerAccumFunc = std::function<void(Accum& accumulator, const RespPm& additional)>;
...
public:
    ...
    SimulatedServer(ServerId _id, int _iops, size_t _thread_pool_size, const ClientRespFunc& _client_resp_f,
                    const ServerAccumFunc& _accum_f, CreateQueueF _create_queue_f)
        : id(_id)
        , priority_queue(
              _create_queue_f(std::bind(&SimulatedServer::has_avail_thread, this),
                              std::bind(&SimulatedServer::inner_post, this, std::placeholders::_1,
        ...
        , accum_f(_accum_f)
    {
        op_time = std::chrono::microseconds((int)(0.5 + thread_pool_size * 1000000.0 / iops));
    ...
    void post(TestRequest&& request, const ClientId& client_id, const ReqPm& req_params, const Cost request_cost)
    {
        time_stats(internal_stats.mtx, internal_stats.add_request_time, [&]() {
            priority_queue->add_request(std::move(request), client_id, req_params, request_cost);
        });
        count_stats(internal_stats.mtx, internal_stats.add_request_count);
    }
...
protected:
    void inner_post(const ClientId& client, std::unique_ptr<TestRequest> request, const RespPm& additional,
                    const Cost request_cost)
    {
        Lock l(inner_queue_mtx);
        assert(!finishing);
        accum_f(accumulator, additional);
        inner_queue.emplace_back(QueueItem(client, std::move(request), additional, request_cost));
    ...
    void run(std::chrono::milliseconds check_period)
    {
        ...
        while (true) {
            ...
            if (!inner_queue.empty()) {
                ...
                time_stats(internal_stats.mtx, internal_stats.request_complete_time, [&]() {
                    priority_queue->request_completed();
                });
                count_stats(internal_stats.mtx, internal_stats.request_complete_count);
