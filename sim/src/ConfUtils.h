...
{
public:
    ConfLine(const std::string& key_, const std::string& val_, const std::string& newsection_,
...
private:
    void load_from_buffer(const char* buf, size_t sz, std::deque<std::string>* errors, std::ostream* warnings);
    static ConfLine* process_line(int line_no, const char* line, std::deque<std::string>* errors);
