...
static_assert(std::is_same<crimson::qos_simulation::Cost, crimson::dmclock::Cost>::value,
              "Please make sure the simulator type crimson::qos_simulation::Cost matches the dmclock type "
              "crimson::dmclock::Cost.");


void test::dmc_server_accumulate_f(test::DmcAccum& a, const test::dmc::PhaseType& phase)
{
...
void test::dmc_client_accumulate_f(test::DmcAccum& a, const test::dmc::PhaseType& phase)
{
    if (test::dmc::PhaseType::reservation == phase) {
        ++a.reservation_count;
    }
