...
public:
    ...
    static inline OrigTracker create(Counter the_delta, Counter the_rho) { return OrigTracker(the_delta, the_rho); }
    ...
    inline Counter get_last_delta() const { return delta_prev_req; }
};   // struct OrigTracker
 ...
 * interface
 */
template<typename S, typename T = OrigTracker> class ServiceTracker
{
...
public:
    ...
    ServiceTracker(std::chrono::duration<Rep, Per> _clean_every, std::chrono::duration<Rep, Per> _clean_age)
        : delta_counter(1)
        , rho_counter(1)
        , clean_age(std::chrono::duration_cast<Duration>(_clean_age))
    {
        cleaning_job =
            std::unique_ptr<RunEvery>(new RunEvery(_clean_every, std::bind(&ServiceTracker::do_clean, this)));
    }
     ...
     */
    void track_resp(const S& server_id, const PhaseType& phase, Counter request_cost = 1u)
    {
        ...
        if (server_map.end() == it) {
            ...
            auto i = server_map.emplace(server_id, T::create(delta_counter, rho_counter));
            it = i.first;
        }
    ...
    ReqParams get_req_params(const S& server)
    {
        ...
        if (server_map.end() == it) {
            server_map.emplace(server, T::create(delta_counter, rho_counter));
...
private:
    ...
    void do_clean()
    {
        ...
        if (earliest > 0) {
            for (auto i = server_map.begin(); i != server_map.end();
                 /* empty */) {
                auto i2 = i++;
