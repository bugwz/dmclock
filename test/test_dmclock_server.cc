...
TEST(dmclock_server, bad_tag_deathtest)
{
    ...
    EXPECT_DEATH_IF_SUPPORTED(Queue(client_info_f, AtLimit::Reject), "Assertion.*Reject.*Delayed")
...
TEST(dmclock_server, client_idle_erase)
{
    ...
    auto client_info_f = [&](ClientId c) -> const dmc::ClientInfo* { return &ci; };
    auto server_ready_f = []() -> bool { return true; };
    auto submit_req_f = [](const ClientId& c, std::unique_ptr<Request> req, dmc::PhaseType phase, uint64_t req_cost) {
        // empty; do nothing
    };
    ...
    auto lock_pq = [&](std::function<void()> code) { test_locked(pq.data_mtx, code); };
    ...
    lock_pq([&]() { EXPECT_EQ(0u, pq.client_map.size()) << "client map initially has size 0"; });
    ...
    lock_pq([&]() {
        EXPECT_EQ(1u, pq.client_map.size()) << "client map has 1 after 1 client";
        EXPECT_FALSE(pq.client_map.at(client)->idle) << "initially client map entry shows not idle.";
    });

    std::this_thread::sleep_for(std::chrono::seconds(6));

    lock_pq([&]() { EXPECT_TRUE(pq.client_map.at(client)->idle) << "after idle age client map entry shows idle."; });

    std::this_thread::sleep_for(std::chrono::seconds(2));

    lock_pq([&]() { EXPECT_EQ(0u, pq.client_map.size()) << "client map loses its entry after erase age"; });
}   // TEST
...
TEST(dmclock_server, add_req_pushprio_queue)
{
    ...
    auto client_info_f = [&](ClientId c) -> const dmc::ClientInfo* { return &ci; };
    auto server_ready_f = []() -> bool { return true; };
    auto submit_req_f = [](const ClientId& c, std::unique_ptr<MyReq> req, dmc::PhaseType phase, uint64_t req_cost) {
        // empty; do nothing
    };
    ...
    lock_pq([&]() {
        EXPECT_EQ(2u, pq.client_map.size()) << "client map has 2 after 2 clients";
        EXPECT_FALSE(pq.client_map.at(client1)->idle) << "initially client1 map entry shows not idle.";
        EXPECT_FALSE(pq.client_map.at(client2)->idle) << "initially client2 map entry shows not idle.";
    });

    // Check client idle state
    std::this_thread::sleep_for(std::chrono::seconds(6));
    lock_pq([&]() {
        EXPECT_TRUE(pq.client_map.at(client1)->idle) << "after idle age client1 map entry shows idle.";
        EXPECT_TRUE(pq.client_map.at(client2)->idle) << "after idle age client2 map entry shows idle.";
    });

    // Sleep until after erase age elapses
    std::this_thread::sleep_for(std::chrono::seconds(2));

    lock_pq([&]() { EXPECT_EQ(0u, pq.client_map.size()) << "client map loses its entries after erase age"; });
}   // TEST
...
TEST(dmclock_server, delayed_tag_calc)
{
    ...
    auto client_info_f = [&](ClientId c) -> const ClientInfo* { return &info; };
...
TEST(dmclock_server, remove_by_req_filter)
{
    ...
    auto client_info_f = [&](ClientId c) -> const dmc::ClientInfo* { return &info1; };
...
TEST(dmclock_server, remove_by_client)
{
    ...
    pq.remove_by_client(client1, true, [&removed](MyReqRef&& r) { removed.push_front(*r); });
    ...
    EXPECT_EQ(0u, pq.request_count()) << "after second client removed, none left";
}   // TEST
...
TEST(dmclock_server, add_req_ref)
{
    ...
    auto client_info_f = [&](ClientId c) -> const dmc::ClientInfo* { return &info; };
...
TEST(dmclock_server_pull, pull_weight)
{
    ...
    EXPECT_EQ(2, c1_count) << "one-third of request should have come from first client";
    EXPECT_EQ(4, c2_count) << "two-thirds of request should have come from second client";
}
...
TEST(dmclock_server_pull, pull_reservation)
{
    ...
    EXPECT_EQ(4, c1_count) << "two-thirds of request should have come from first client";
    EXPECT_EQ(2, c2_count) << "one-third of request should have come from second client";
...
TEST(dmclock_server_pull, update_client_info)
{
    ...
    EXPECT_EQ(2, c1_count) << "before: one-third of request should have come from first client";
    EXPECT_EQ(4, c2_count) << "before: two-thirds of request should have come from second client";
    ...
    EXPECT_EQ(3, c1_count) << "after: one-third of request should have come from first client";
    EXPECT_EQ(3, c2_count) << "after: two-thirds of request should have come from second client";
}
...
TEST(dmclock_server_pull, dynamic_cli_info_f)
{
    ...
    dmc::ClientInfo info1[] = {dmc::ClientInfo(0.0, 100.0, 0.0), dmc::ClientInfo(0.0, 150.0, 0.0)};
    dmc::ClientInfo info2[] = {dmc::ClientInfo(0.0, 200.0, 0.0), dmc::ClientInfo(0.0, 50.0, 0.0)};
    ...
    auto client_info_f = [&](ClientId c) -> const dmc::ClientInfo* {
        if (client1 == c)
            return &info1[cli_info_group];
        else if (client2 == c)
            return &info2[cli_info_group];
        else {
            ADD_FAILURE() << "client info looked up for non-existent client";
            return nullptr;
        }
    ...
    auto run_test = [&](float lower_bound, float upper_bound) {
        ...
        float prop1 =
            float(info1[cli_info_group].weight) / (info1[cli_info_group].weight + info2[cli_info_group].weight);
        float prop2 =
            float(info2[cli_info_group].weight) / (info1[cli_info_group].weight + info2[cli_info_group].weight);
    ...
    run_test(0.0F /* lower bound */, 0.5F /* upper bound */);
    ...
    // new client info.
    cli_info_group = 1;
    run_test(1.0F / 3 /* lower bound */, 2.0F / 3 /* upper bound */);
}
...
TEST(dmclock_server_pull, ready_and_under_limit)
{
    ...
    pr = pq->pull_request(start_time + 0.5);
    EXPECT_EQ(Queue::NextReqType::future, pr.type) << "too soon for next reservation";
...
TEST(dmclock_server_pull, pull_wait_at_limit)
{
    ...
    EXPECT_EQ(1, c1_count) << "one request should have come from first client";
    EXPECT_EQ(1, c2_count) << "one request should have come from second client";
    ...
    EXPECT_EQ(50, c1_count) << "half of the total requests should have come from first client";
    EXPECT_EQ(2, c2_count) << "only two requests should have come from second client";
